...
func TestBasicInsert(t *testing.T) {
	...
	for _, w := range []int{128, 256, 512, 1024, 2048, 4096} {
		t.Run(fmt.Sprintf("BasicInsert%d", w), func(t *testing.T) {
...
func TestDirBuilding(t *testing.T) {
	...
	if k.B58String() != "QmY89TkSEVHykWMHDmyejSWFj9CYNtvzw4UwnT9xbc4Zjc" {
...
func printDag(ds dag.DAGService, nd *dag.Node, depth int) {
	...
	for _, l := range nd.Links {
		...
		ch, err := ds.Get(context.Background(), cid.NewCidV0(l.Hash))
		if err != nil {
			panic(err)
		}
...
func BenchmarkHAMTInsert(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		err = s.Insert(context.TODO(), fmt.Sprint(i), ft.EmptyDirNode())
		if err != nil {
			b.Fatal(err)
		}
